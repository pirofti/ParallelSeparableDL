...
def omp_2D(dictionary, samples, n_nonzero_coefs, params=[]):
    ...
    samples_vec = np.zeros((samples.shape[1] * samples.shape[2],
                            samples.shape[0]))
    for i in range(samples.shape[0]):
        samples_vec[:, i] = samples[i, :, :].T.reshape(
    ...
    codes = np.zeros((samples.shape[0],
                      dictionary[0].shape[1], dictionary[1].shape[1]))
    for i in range(samples.shape[0]):
